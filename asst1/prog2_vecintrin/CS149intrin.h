...
void addUserLog(const char * logStr, const __cs149_mask& masks);
inline void printMask(const char* logStr, const __cs149_mask& masks)
{
  printf("%s: ", logStr);
  for (int i = 0; i < VECTOR_WIDTH; ++i)
  {
    printf("%d ", masks.value[i]);
  }
  printf("\n");
}

inline void printVecInt(const char* logStr, const __cs149_vec_int vecint)
{
  printf("%s: ", logStr);
  for (int i = 0; i < VECTOR_WIDTH; ++i)
  {
    printf("%d ", vecint.value[i]);
  }
  printf("\n");
}

inline void printVecFloat(const char* logStr, const __cs149_vec_float vecint)
{
  printf("%s: ", logStr);
  for (int i = 0; i < VECTOR_WIDTH; ++i)
  {
    printf("%f ", vecint.value[i]);
  }
  printf("\n");
}
#endif
