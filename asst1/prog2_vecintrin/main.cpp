...
bool verifyResult(float* values, int* exponents, float* output, float* gold, int N) {
  ...
  if (incorrect != -1) {
    if (incorrect >= N)
      printf("You have written to out of bound value! index %d\n", incorrect);
...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  const static float clamped = 9.999999f;
  __cs149_vec_float clampValue = _cs149_vset_float(clamped);

  __cs149_mask maskAll, maskNeedMul, maskNeedClamp;
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH)
  {
    ...
    _cs149_vgt_int(maskNeedMul, y, zeroInt, maskAll);
    
    while (_cs149_cntbits(maskNeedMul))
    {
      _cs149_vmult_float(result, result, x, maskNeedMul);
      _cs149_vsub_int(y, y, oneInt, maskNeedMul);
      _cs149_vgt_int(maskNeedMul, y, zeroInt, maskAll);
    }

    _cs149_vgt_float(maskNeedClamp, result, clampValue, maskAll);
    _cs149_vset_float(result, clamped, maskNeedClamp);

    if ((N - i) < VECTOR_WIDTH)
    {
      __cs149_mask tailRemoveMaks = _cs149_init_ones(N - i);
      tailRemoveMaks = _cs149_mask_not(tailRemoveMaks);

      // addUserLog("tailRemoveMasks", tailRemoveMaks);
      // CS149Logger.printLog();
      
      _cs149_vset_float(result, 0.f, tailRemoveMaks);
    }
    _cs149_vstore_float(output + i, result, maskAll);
  }
...
float arraySumVector(float* values, int N) {
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    // printVecFloat("temp", temp);
    _cs149_vadd_float(sumvec, sumvec, temp,  maskAll);
    // printVecFloat("sumvec", sumvec);
  }
